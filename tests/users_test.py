...
class TestUsers:
    ...
    def test_user_logining(self, api, time_response):
        response = api.user.user_login(
            User.USER_TEST_DATA.value.get("username"), User.USER_TEST_DATA.value.get("password"))
        assert response.status_code == 200
        user = response.json()
        message = user.get("message")
        assert "logged in user session:" in message
        self.USER_SESSION = message.split(":")[1]
    ...
    def test_update_user(self, api, time_response):
        ...
        response = api.user.update_user(current_name, future_data)
        assert response.status_code == 200

    @pytest.mark.medium
    def test_check_barsik_data(self, api, time_response):
        response = api.user.get_user(User.USER_Barsik_DATA.value.get("username"))
        assert response.status_code == 200
        barsik = response.json()
